          ...
          {navLinks.map((link) => (
            <Link key={link.path} to={link.path}>
              <Button
                ...
                className={currentPath === link.path ? 'bg-primary text-primary-foreground hover:bg-primary/90' : 'text-foreground hover:text-foreground hover:bg-accent'}
              >
                {link.label}
              </Button>
            </Link>
          ...
          <div className="container mx-auto flex flex-col gap-1 px-4 py-4">
            {navLinks.map((link) => (
              <Link key={link.path} to={link.path} onClick={() => setMobileMenuOpen(false)}>
                <Button
                  variant={currentPath === link.path ? 'default' : 'ghost'}
                  className={`w-full justify-start ${currentPath === link.path ? 'bg-primary text-primary-foreground hover:bg-primary/90' : 'text-foreground hover:text-foreground hover:bg-accent'}`}
                >
                  {link.label}
                </Button>
              </Link>
